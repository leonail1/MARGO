...
#pragma omp parallel for schedule(static, 65536)
        ...
        bool search_invocation)
    {
        ...
        for (auto id : init_ids)
        {
            ...
            if (fast_iterate)
            {
                // 使用位图检查节点是否已访问，每个元素占一个字节
                if (inserted_into_pool_bs[id] == 0)
                {
                    inserted_into_pool_bs[id] = 1; // 标记为已访问
                    best_L_nodes[l++] = nn;
                }
            }
            else
            {
                // 使用哈希表检查节点是否已访问（处理大规模数据集）
                if (inserted_into_pool_rs.find(id) == inserted_into_pool_rs.end())
                {
                    inserted_into_pool_rs.insert(id); // 插入到已访问集合
                    best_L_nodes[l++] = nn;
                }
            }
            if (l == Lsize)
        ...
        while (k < l)
        {
            ...
            {                                 // flag=true表示未扩展
                ...
                // 因为贪心算法要求始终扩展距离最近的点，如果有更近的点插入，则该点必然比第 k+1 个点更近
                if (nk <= k)
                    k = nk;
                else
                    ++k;
...
#pragma omp parallel for num_threads(num_threads) schedule(dynamic, 8192)
    ...
    template <typename T, typename TagT>
    void Index<T, TagT>::optimize_index_layout()
