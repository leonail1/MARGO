        ...
        for (_s64 node_ctr = 0; node_ctr < (_s64)(visit_order.size()); node_ctr++)
        {
            ...
            if (this->_final_graph[node].size() > this->_indexingRange)
            {
                ...
                for (auto cur_nbr : this->_final_graph[node])
                {
                    ...
                    {
                        float dist =
                            (this->_distance)->compare(this->_data + this->_aligned_dim * (size_t)node, this->_data + this->_aligned_dim * (size_t)cur_nbr, (unsigned)this->_aligned_dim);
                        dummy_pool.emplace_back(Neighbor(cur_nbr, dist, true));
                        dummy_visited.insert(cur_nbr);
                    }
                }
        ...
        while (cur_alpha <= alpha && result.size() < degree)
        {
                 ...
                 result.size() < degree && iter != pool.end(); ++iter)
            {
                ...
                for (auto iter2 = iter + 1; iter2 != pool.end(); iter2++)
                {
                    ...
                    // iter2 对应 p'
                    float djk =
                        (this->_distance)->compare(this->_data + this->_aligned_dim * (size_t)iter2->id, this->_data + this->_aligned_dim * (size_t)iter->id, (unsigned)this->_aligned_dim);
